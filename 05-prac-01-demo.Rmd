...
```{r}
Londonborough <- st_read("wk1/data/london_borough/data/statistical-gis-boundaries-london/ESRI/London_Borough_Excluding_MHW.shp") %>% st_transform(.,27700)
...
Worldcities <-st_read("wk3/data_IPCC/World_Cities/World_Cities.shp") %>% 
  st_transform(.,27700) 

UK_outline <- st_read("wk5/data/gadm41_GBR_shp/gadm41_GBR_0.shp") %>%
  st_transform(., 27700)
...
`st_bbox` is a function that creates a bounding box
this is a box that covers the extent of our data

`st_as_sf`c` converts our bounding box into a polygon
this is so we can plot it
...
```{r}
tmap_save(t, 'wk5/hotelsandairbnbR.png')

tmap_save(main,insets_tm = inset,insets_vp=viewport(x=0.86, y=0.29, width=.5, height=.55), filename="wk5/test.pdf", dpi=600)
```
...
## Interactive map with pop up boxes
